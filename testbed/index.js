...
import {
  Vec2 as indexjs_Vec2,
  AABB as indexjs_AABB,
  MouseJoint as indexjs_MouseJoint,
} from "../lib/";
...
planck.testbed = function(opts, callback) {
  ...
  Stage(function(stage, canvas) {
    ...
    function findBody(point) {
      var body;
      var aabb = indexjs_AABB(point, point);
    ...
    viewer.attr('spy', true).on(Stage.Mouse.START, function(point) {
      ...
      if (testbed.mouseForce) {
        ...
        mouseJoint = indexjs_MouseJoint({maxForce: 1000}, mouseGround, body, Vec2(point));
