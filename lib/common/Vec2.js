...
Vec2.lengthOf = function(v) {
  _ASSERT && Vec2.assert(v);
  return Math_math.sqrt(v.x * v.x + v.y * v.y);
}
...
Vec2.distance = function(v, w) {
  ...
  return Math_math.sqrt(dx * dx + dy * dy);
}
...
Vec2.abs = function(v) {
  _ASSERT && Vec2.assert(v);
  return Vec2.neo(Math_math.abs(v.x), Math_math.abs(v.y));
}
...
Vec2.upper = function(v, w) {
  _ASSERT && Vec2.assert(v);
  _ASSERT && Vec2.assert(w);
  return Vec2.neo(Math_math.max(v.x, w.x), Math_math.max(v.y, w.y));
}

Vec2.lower = function(v, w) {
  _ASSERT && Vec2.assert(v);
  _ASSERT && Vec2.assert(w);
  return Vec2.neo(Math_math.min(v.x, w.x), Math_math.min(v.y, w.y));
}
