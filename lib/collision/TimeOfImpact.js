...
import { now as Timerjs_now, diff as Timerjs_diff } from "../util/Timer";
...
stats_toString.toiTime = 0;
stats_toString.toiMaxTime = 0;
stats_toString.toiCalls = 0;
stats_toString.toiIters = 0;
stats_toString.toiMaxIters = 0;
stats_toString.toiRootIters = 0;
stats_toString.toiMaxRootIters = 0;
...
function TimeOfImpact(output, input) {
  var timer = Timerjs_now();

  ++stats_toString.toiCalls;
  ...
  _ASSERT && common_assert(target > tolerance);
  ...
  for (;;) {
    ...
    for (;;) {
      ...
      stats_toString.toiMaxRootIters = Math.max(stats_toString.toiMaxRootIters, rootIterCount);
  ...
  stats_toString.toiMaxIters = Math.max(stats_toString.toiMaxIters, iter);

  var time = Timerjs_diff(timer);
  stats_toString.toiMaxTime = Math.max(stats_toString.toiMaxTime, time);
  stats_toString.toiTime += time;
}
...
SeparationFunction.prototype.initialize = function(cache, proxyA, sweepA, proxyB, sweepB, t1) {
  ...
  _ASSERT && common_assert(0 < count && count < 3);
