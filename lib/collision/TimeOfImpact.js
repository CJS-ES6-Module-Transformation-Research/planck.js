...
import { assert as TimeOfImpact_assert } from "../util/common";
import { now as TimeOfImpact_now } from "../util/Timer";
import { diff as TimeOfImpact_diff } from "../util/Timer";
...
function TimeOfImpact(output, input) {
  ...
  _ASSERT && TimeOfImpact_assert(target > tolerance);
  ...
  var time = TimeOfImpact_diff(timer);
...
SeparationFunction.prototype.initialize = function(cache, proxyA, sweepA, proxyB, sweepB, t1) {
  ...
  _ASSERT && TimeOfImpact_assert(0 < count && count < 3);
...
SeparationFunction.prototype.compute = function(find, t) {
  ...
  default:
    _ASSERT && TimeOfImpact_assert(false);
    if (find) {
      this.indexA = -1;
      this.indexB = -1;
    }
