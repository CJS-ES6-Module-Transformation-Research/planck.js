...
function Distance(output, cache, input) {
  ...
  var k_maxIters = _Settings.Settings.maxDistnceIterations;
  ...
  while (iter < k_maxIters) {
    ...
    if (d.lengthSquared() < _Math.math.EPSILON * _Math.math.EPSILON) {
  ...
  if (input.useRadii) {
    ...
    if (output.distance > rA + rB && output.distance > _Math.math.EPSILON) {
...
Simplex.prototype.readCache = function (cache, proxyA, transformA, proxyB, transformB) {
  ...
  if (this.m_count > 1) {
    ...
    if (metric2 < 0.5 * metric1 || 2.0 * metric1 < metric2 || metric2 < _Math.math.EPSILON) {
      // Reset the simplex.
      this.m_count = 0;
    }
  }
...
Distance.testOverlap = function (shapeA, indexA, shapeB, indexB, xfA, xfB) {
  ...
  return output.distance < 10.0 * _Math.math.EPSILON;
};
 ...
 */
exports.Distance = Distance_Distance;