...
Solver.prototype.addBody = function (body) {
  _ASSERT && _common.assert.assert(body instanceof _Body.Body, 'Not a Body!', body);
...
Solver.prototype.addContact = function (contact) {
  _ASSERT && _common.assert.assert(contact instanceof _Contact.Contact, 'Not a Contact!', contact);
  this.m_contacts.push(contact);
};

Solver.prototype.addJoint = function (joint) {
  _ASSERT && _common.assert.assert(joint instanceof _Joint.Joint, 'Not a Joint!', joint);
...
Solver.prototype.solveWorld = function (step) {
  ...
  for (var seed = world.m_bodyList; seed; seed = seed.m_next) {
    ...
    while (stack.length > 0) {
      ...
      var b = stack.pop();
      _ASSERT && _common.assert.assert(b.isActive() == true);
      this.addBody(b);
...
Solver.prototype.solveIsland = function (step) {
  ...
  for (var i = 0; i < this.m_bodies.length; ++i) {
    ...
    if (rotation * rotation > _Settings.maxRotationSquared) {
      var ratio = _Settings.maxRotation / Math.abs(rotation);
      w *= ratio;
    }
...
Solver.prototype.printBodies = function (tag) {
  for (var i = 0; i < this.m_bodies.length; ++i) {
    var b = this.m_bodies[i];
    _common.assert.debug(tag, b.c_position.a, b.c_position.c.x, b.c_position.c.y, b.c_velocity.w, b.c_velocity.v.x, b.c_velocity.v.y);
  }
};
  ...
  for (;;) {
    ...
    for (var c = world.m_contactList; c; c = c.m_next) {
      ...
      if (c.m_toiFlag) {
        ...
        _ASSERT && _common.assert.assert(bA.isDynamic() || bB.isDynamic());
        ...
        _ASSERT && _common.assert.assert(alpha0 < 1.0);
