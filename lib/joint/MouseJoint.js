...
MouseJoint._super = _Joint2.default;
MouseJoint.prototype = (0, _create2.default)(MouseJoint._super.prototype);
...
function MouseJoint(def, bodyA, bodyB, target) {
  ...
  _ASSERT && common.assert(_Math2.default.isFinite(def.maxForce) && def.maxForce >= 0.0);
  _ASSERT && common.assert(_Math2.default.isFinite(def.frequencyHz) && def.frequencyHz >= 0.0);
  _ASSERT && common.assert(_Math2.default.isFinite(def.dampingRatio) && def.dampingRatio >= 0.0);

  this.m_targetA = target ? _Vec2.default.clone(target) : def.target || _Vec2.default.zero();
  this.m_localAnchorB = _Transform2.default.mulTVec2(bodyB.getTransform(), this.m_targetA);

  this.m_maxForce = def.maxForce;
  this.m_impulse = _Vec2.default.zero();
  ...
  // Solver temp
  this.m_rB = _Vec2.default.zero();
  this.m_localCenterB = _Vec2.default.zero();
  this.m_invMassB = 0.0;
  this.m_invIB = 0.0;
  this.mass = new _Mat2.default();
  this.m_C = _Vec2.default.zero();
...
MouseJoint.prototype.setTarget = function (target) {
  ...
  this.m_targetA = _Vec2.default.clone(target);
};
...
MouseJoint.prototype.getAnchorA = function () {
  return _Vec2.default.clone(this.m_targetA);
};
...
MouseJoint.prototype.getReactionForce = function (inv_dt) {
  return _Vec2.default.mul(inv_dt, this.m_impulse);
};
...
MouseJoint.prototype.initVelocityConstraints = function (step) {
  ...
  var h = step.dt;
  _ASSERT && common.assert(d + h * k > _Math2.default.EPSILON);
  ...
  if (step.warmStarting) {
    ...
    wB += this.m_invIB * _Vec2.default.cross(this.m_rB, this.m_impulse);
  } else {
...
MouseJoint.prototype.solveVelocityConstraints = function (step) {
  var velocity = this.m_bodyB.c_velocity;
  var vB = _Vec2.default.clone(velocity.v);
  var wB = velocity.w;

  // Cdot = v + cross(w, r)

  var Cdot = _Vec2.default.cross(wB, this.m_rB);
  Cdot.add(vB);

  Cdot.addCombine(1, this.m_C, this.m_gamma, this.m_impulse);
  Cdot.neg();

  var impulse = _Mat2.default.mulVec2(this.m_mass, Cdot);

  var oldImpulse = _Vec2.default.clone(this.m_impulse);
  this.m_impulse.add(impulse);
  var maxImpulse = step.dt * this.m_maxForce;
  this.m_impulse.clamp(maxImpulse);
  impulse = _Vec2.default.sub(this.m_impulse, oldImpulse);

  vB.addMul(this.m_invMassB, impulse);
  wB += this.m_invIB * _Vec2.default.cross(this.m_rB, impulse);
