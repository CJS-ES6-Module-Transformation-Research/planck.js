import { debug as MouseJoint_debug } from "../util/common";
...
function MouseJoint(def, bodyA, bodyB, target) {
  ...
  _ASSERT && common.assert(Mathjs_isFinite(def.maxForce) && def.maxForce >= 0.0);
  _ASSERT && common.assert(Mathjs_isFinite(def.frequencyHz) && def.frequencyHz >= 0.0);
  _ASSERT && common.assert(Mathjs_isFinite(def.dampingRatio) && def.dampingRatio >= 0.0);
...
MouseJoint.prototype.initVelocityConstraints = function(step) {
  ...
  var h = step.dt;
  _ASSERT && common.assert(d + h * k > Mathjs_EPSILON);
