...
DistanceJoint._super = _Joint2.default;
DistanceJoint.prototype = (0, _create2.default)(DistanceJoint._super.prototype);
...
function DistanceJoint(def, bodyA, bodyB, anchorA, anchorB) {
  ...
  // Solver shared
  this.m_localAnchorA = anchorA ? bodyA.getLocalPoint(anchorA) : def.localAnchorA || _Vec2.default.zero();
  this.m_localAnchorB = anchorB ? bodyB.getLocalPoint(anchorB) : def.localAnchorB || _Vec2.default.zero();
  this.m_length = _Math2.default.isFinite(def.length) ? def.length : _Vec2.default.distance(bodyA.getWorldPoint(this.m_localAnchorA), bodyB.getWorldPoint(this.m_localAnchorB));
...
DistanceJoint.prototype.getReactionForce = function (inv_dt) {
  return _Vec2.default.mul(this.m_impulse, this.m_u).mul(inv_dt);
};
...
DistanceJoint.prototype.initVelocityConstraints = function (step) {
  ...
  this.m_u = _Vec2.default.sub(_Vec2.default.add(cB, this.m_rB), _Vec2.default.add(cA, this.m_rA));

  // Handle singularity.
  var length = this.m_u.length();
  if (length > _Settings2.default.linearSlop) {
    this.m_u.mul(1.0 / length);
  } else {
    this.m_u.set(0.0, 0.0);
  }

  var crAu = _Vec2.default.cross(this.m_rA, this.m_u);
  var crBu = _Vec2.default.cross(this.m_rB, this.m_u);
  ...
  if (this.m_frequencyHz > 0.0) {
    ...
    // Frequency
    var omega = 2.0 * _Math2.default.PI * this.m_frequencyHz;
  ...
  if (step.warmStarting) {
    ...
    var P = _Vec2.default.mul(this.m_impulse, this.m_u);

    vA.subMul(this.m_invMassA, P);
    wA -= this.m_invIA * _Vec2.default.cross(this.m_rA, P);

    vB.addMul(this.m_invMassB, P);
    wB += this.m_invIB * _Vec2.default.cross(this.m_rB, P);
  } else {
    this.m_impulse = 0.0;
  }
...
DistanceJoint.prototype.solveVelocityConstraints = function (step) {
  ...
  var Cdot = _Vec2.default.dot(this.m_u, vpB) - _Vec2.default.dot(this.m_u, vpA);

  var impulse = -this.m_mass * (Cdot + this.m_bias + this.m_gamma * this.m_impulse);
  this.m_impulse += impulse;

  var P = _Vec2.default.mul(impulse, this.m_u);
  vA.subMul(this.m_invMassA, P);
  wA -= this.m_invIA * _Vec2.default.cross(this.m_rA, P);
  vB.addMul(this.m_invMassB, P);
  wB += this.m_invIB * _Vec2.default.cross(this.m_rB, P);
...
DistanceJoint.prototype.solvePositionConstraints = function (step) {
  ...
  var rA = _Rot2.default.mulSub(qA, this.m_localAnchorA, this.m_localCenterA);
  var rB = _Rot2.default.mulSub(qB, this.m_localAnchorB, this.m_localCenterB);
  var u = _Vec2.default.sub(_Vec2.default.add(cB, rB), _Vec2.default.add(cA, rA));

  var length = u.normalize();
  var C = length - this.m_length;
  C = _Math2.default.clamp(C, -_Settings2.default.maxLinearCorrection, _Settings2.default.maxLinearCorrection);

  var impulse = -this.m_mass * C;
  var P = _Vec2.default.mul(impulse, u);

  cA.subMul(this.m_invMassA, P);
  aA -= this.m_invIA * _Vec2.default.cross(rA, P);
  cB.addMul(this.m_invMassB, P);
  aB += this.m_invIB * _Vec2.default.cross(rB, P);
  ...
  return _Math2.default.abs(C) < _Settings2.default.linearSlop;
};
