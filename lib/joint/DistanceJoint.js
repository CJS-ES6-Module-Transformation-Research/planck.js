...
DistanceJoint._super = Joint_Jointjs;
DistanceJoint.prototype = utilcreate_createjsjs(DistanceJoint._super.prototype);
...
function DistanceJoint(def, bodyA, bodyB, anchorA, anchorB) {
  ...
  // Solver shared
  this.m_localAnchorA = anchorA ? bodyA.getLocalPoint(anchorA) : def.localAnchorA || commonVec2_Vec2js.zero();
  this.m_localAnchorB = anchorB ? bodyB.getLocalPoint(anchorB) : def.localAnchorB || commonVec2_Vec2js.zero();
  this.m_length = commonMath_mathjs.isFinite(def.length) ? def.length :
    commonVec2_Vec2js.distance(bodyA.getWorldPoint(this.m_localAnchorA), bodyB.getWorldPoint(this.m_localAnchorB));
...
DistanceJoint.prototype.initVelocityConstraints = function(step) {
  ...
  this.m_u = commonVec2_Vec2js.sub(commonVec2_Vec2js.add(cB, this.m_rB), commonVec2_Vec2js.add(cA, this.m_rA));

  // Handle singularity.
  var length = this.m_u.length();
  if (length > Settings_Settingsjs.linearSlop) {
    this.m_u.mul(1.0 / length);
  } else {
    this.m_u.set(0.0, 0.0);
  }

  var crAu = commonVec2_Vec2js.cross(this.m_rA, this.m_u);
  var crBu = commonVec2_Vec2js.cross(this.m_rB, this.m_u);
...
DistanceJoint.prototype.solveVelocityConstraints = function(step) {
  ...
  var Cdot = commonVec2_Vec2js.dot(this.m_u, vpB) - commonVec2_Vec2js.dot(this.m_u, vpA);

  var impulse = -this.m_mass
      * (Cdot + this.m_bias + this.m_gamma * this.m_impulse);
  this.m_impulse += impulse;

  var P = commonVec2_Vec2js.mul(impulse, this.m_u);
  vA.subMul(this.m_invMassA, P);
  wA -= this.m_invIA * commonVec2_Vec2js.cross(this.m_rA, P);
  vB.addMul(this.m_invMassB, P);
  wB += this.m_invIB * commonVec2_Vec2js.cross(this.m_rB, P);
...
DistanceJoint.prototype.solvePositionConstraints = function(step) {
  ...
  var u = commonVec2_Vec2js.sub(commonVec2_Vec2js.add(cB, rB), commonVec2_Vec2js.add(cA, rA));

  var length = u.normalize();
  var C = length - this.m_length;
  C = commonMath_mathjs
      .clamp(C, -Settings_Settingsjs.maxLinearCorrection, Settings_Settingsjs.maxLinearCorrection);

  var impulse = -this.m_mass * C;
  var P = commonVec2_Vec2js.mul(impulse, u);
