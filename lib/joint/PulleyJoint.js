...
function PulleyJoint(def, bodyA, bodyB, groundA, groundB, anchorA, anchorB, ratio) {
  ...
  this.m_lengthA = commonMath_mathjs.isFinite(def.lengthA) ? def.lengthA : commonVec2_Vec2js.distance(anchorA, groundA);
  this.m_lengthB = commonMath_mathjs.isFinite(def.lengthB) ? def.lengthB : commonVec2_Vec2js.distance(anchorB, groundB);
  this.m_ratio = commonMath_mathjs.isFinite(ratio) ? ratio : def.ratio;

  _ASSERT && utilcommon_assertjs.assert(ratio > commonMath_mathjs.EPSILON);
...
PulleyJoint.prototype.initVelocityConstraints = function(step) {
  ...
  if (lengthA > 10.0 * Settings_Settingsjs.linearSlop) {
    this.m_uA.mul(1.0 / lengthA);
  } else {
    this.m_uA.setZero();
  }

  if (lengthB > 10.0 * Settings_Settingsjs.linearSlop) {
