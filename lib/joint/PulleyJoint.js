import { assert as PulleyJoint_common } from "../util/common";
import { optionsjs as PulleyJoint_options } from "../util/options";
import { createjs as PulleyJoint_create } from "../util/create";

import {
  math as PulleyJoint_Math,
  EPSILON as Mathjs_EPSILON,
  isFinite as Mathjs_isFinite,
} from "../common/Math";

import {
  Vec2 as PulleyJoint_Vec2,
  neo as Vec2js_neo,
...
import { Rot as PulleyJoint_Rot, neo as Rotjs_neo, mulVec2 as Rotjs_mulVec2 } from "../common/Rot";
import { Sweep as PulleyJoint_Sweep } from "../common/Sweep";
import { Joint as PulleyJoint_Joint } from "../Joint";
...
PulleyJoint_MIN_PULLEY_LENGTH = 2.0;
PulleyJoint__super = PulleyJoint_Joint;
PulleyJoint.prototype = PulleyJoint_create(PulleyJoint__super.prototype);
...
function PulleyJoint(def, bodyA, bodyB, groundA, groundB, anchorA, anchorB, ratio) {
  ...
  def = PulleyJoint_options(def, PulleyJointDef);
  PulleyJoint_Joint.call(this, def, bodyA, bodyB);
  bodyA = this.m_bodyA;
  bodyB = this.m_bodyB;
  ...
  _ASSERT && PulleyJoint_common(ratio > Mathjs_EPSILON);
...
PulleyJoint.prototype.initVelocityConstraints = function(step) {
  ...
  if (lengthA > 10.0 * Settings) {
    this.m_uA.mul(1.0 / lengthA);
  } else {
    this.m_uA.setZero();
  }

  if (lengthB > 10.0 * Settings) {
