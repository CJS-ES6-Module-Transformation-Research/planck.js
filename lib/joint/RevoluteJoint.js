...
RevoluteJoint._super = _Joint2.default;
RevoluteJoint.prototype = (0, _create2.default)(RevoluteJoint._super.prototype);
...
RevoluteJoint.prototype.getReactionForce = function (inv_dt) {
  return _Vec2.default.neo(this.m_impulse.x, this.m_impulse.y).mul(inv_dt);
};
...
RevoluteJoint.prototype.initVelocityConstraints = function (step) {
  ...
  if (this.m_enableLimit && fixedRotation == false) {
    ...
    if (_Math2.default.abs(this.m_upperAngle - this.m_lowerAngle) < 2.0 * _Settings2.default.angularSlop) {
  ...
  if (step.warmStarting) {
    ...
    var P = _Vec2.default.neo(this.m_impulse.x, this.m_impulse.y);

    vA.subMul(mA, P);
    wA -= iA * (_Vec2.default.cross(this.m_rA, P) + this.m_motorImpulse + this.m_impulse.z);

    vB.addMul(mB, P);
    wB += iB * (_Vec2.default.cross(this.m_rB, P) + this.m_motorImpulse + this.m_impulse.z);
  } else {
...
RevoluteJoint.prototype.solveVelocityConstraints = function (step) {
  ...
  if (this.m_enableLimit && this.m_limitState != inactiveLimit && fixedRotation == false) {
    ...
    Cdot1.addCombine(1, vB, 1, _Vec2.default.cross(wB, this.m_rB));
    Cdot1.subCombine(1, vA, 1, _Vec2.default.cross(wA, this.m_rA));
    var Cdot2 = wB - wA; // float
    var Cdot = (0, _Vec4.default)(Cdot1.x, Cdot1.y, Cdot2);

    var impulse = _Vec4.default.neg(this.m_mass.solve33(Cdot)); // Vec3
    ...
    if (this.m_limitState == equalLimits) {
      ...
      if (newImpulse < 0.0) {
        var rhs = _Vec2.default.combine(-1, Cdot1, this.m_impulse.z, _Vec2.default.neo(this.m_mass.ez.x, this.m_mass.ez.y)); // Vec2
    ...
    var P = _Vec2.default.neo(impulse.x, impulse.y);

    vA.subMul(mA, P);
    wA -= iA * (_Vec2.default.cross(this.m_rA, P) + impulse.z);

    vB.addMul(mB, P);
    wB += iB * (_Vec2.default.cross(this.m_rB, P) + impulse.z);
  } else {
    // Solve point-to-point constraint
    var Cdot = _Vec2.default.zero();
    Cdot.addCombine(1, vB, 1, _Vec2.default.cross(wB, this.m_rB));
    Cdot.subCombine(1, vA, 1, _Vec2.default.cross(wA, this.m_rA));
    var impulse = this.m_mass.solve22(_Vec2.default.neg(Cdot)); // Vec2
...
RevoluteJoint.prototype.solvePositionConstraints = function (step) {
  ...
  if (this.m_enableLimit && this.m_limitState != inactiveLimit && fixedRotation == false) {
    ...
    if (this.m_limitState == equalLimits) {
      ...
      var C = _Math2.default.clamp(angle - this.m_lowerAngle, -_Settings2.default.maxAngularCorrection, _Settings2.default.maxAngularCorrection); // float
      limitImpulse = -this.m_motorMass * C;
      angularError = _Math2.default.abs(C);
    } else if (this.m_limitState == atLowerLimit) {
      var C = angle - this.m_lowerAngle; // float
      angularError = -C;

      // Prevent large angular corrections and allow some slop.
      C = _Math2.default.clamp(C + _Settings2.default.angularSlop, -_Settings2.default.maxAngularCorrection, 0.0);
      limitImpulse = -this.m_motorMass * C;
    } else if (this.m_limitState == atUpperLimit) {
      ...
      // Prevent large angular corrections and allow some slop.
      C = _Math2.default.clamp(C - _Settings2.default.angularSlop, 0.0, _Settings2.default.maxAngularCorrection);
      limitImpulse = -this.m_motorMass * C;
    }
  ...
  // Solve point-to-point constraint.
  {
    ...
    var impulse = _Vec2.default.neg(K.solve(C)); // Vec2

    cA.subMul(mA, impulse);
    aA -= iA * _Vec2.default.cross(rA, impulse);

    cB.addMul(mB, impulse);
    aB += iB * _Vec2.default.cross(rB, impulse);
  }
  ...
  return positionError <= _Settings2.default.linearSlop && angularError <= _Settings2.default.angularSlop;
};
