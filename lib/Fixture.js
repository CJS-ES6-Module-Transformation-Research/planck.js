import { assert as Fixture_common } from "./util/common";
import { optionsjs as Fixture_options } from "./util/options";
import { math as Fixture_Math, isFinite as Mathjs_isFinite } from "./common/Math";
import { Vec2 as Fixture_Vec2, sub as Vec2js_sub } from "./common/Vec2";
import { AABB as Fixture_AABB } from "./collision/AABB";
/*
...
function Fixture(body, shape, def) {
  ...
  def = Fixture_options(def, FixtureDef);
...
Fixture.prototype.setDensity = function(density) {
  _ASSERT && Fixture_common.assert(Mathjs_isFinite(density) && density >= 0.0);
...
Fixture.prototype.getAABB = function(childIndex) {
  _ASSERT && Fixture_common.assert(0 <= childIndex && childIndex < this.m_proxyCount);
...
Fixture.prototype.createProxies = function(broadPhase, xf) {
  _ASSERT && Fixture_common.assert(this.m_proxyCount == 0);
...
Fixture.prototype.synchronize = function(broadPhase, xf1, xf2) {
  for (var i = 0; i < this.m_proxyCount; ++i) {
    ...
    // effect).
    var aabb1 = new Fixture_AABB();
    var aabb2 = new Fixture_AABB();
    this.m_shape.computeAABB(aabb1, xf1, proxy.childIndex);
    this.m_shape.computeAABB(aabb2, xf2, proxy.childIndex);

    proxy.aabb.combine(aabb1, aabb2);

    var displacement = Vec2js_sub(xf2.p, xf1.p);
 ...
 */
export { exported_Fixture as Fixture };
