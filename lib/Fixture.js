...
function FixtureProxy(fixture, childIndex) {
  this.aabb = new collisionAABB_AABBjs();
...
function Fixture(body, shape, def) {
  ...
  def = utiloptions_optionsjsjs(def, FixtureDef);
...
Fixture.prototype.setDensity = function(density) {
  _ASSERT && utilcommon_assertjs.assert(commonMath_mathjs.isFinite(density) && density >= 0.0);
...
Fixture.prototype.getAABB = function(childIndex) {
  _ASSERT && utilcommon_assertjs.assert(0 <= childIndex && childIndex < this.m_proxyCount);
...
Fixture.prototype.createProxies = function(broadPhase, xf) {
  _ASSERT && utilcommon_assertjs.assert(this.m_proxyCount == 0);
...
Fixture.prototype.synchronize = function(broadPhase, xf1, xf2) {
  for (var i = 0; i < this.m_proxyCount; ++i) {
    ...
    // effect).
    var aabb1 = new collisionAABB_AABBjs();
    var aabb2 = new collisionAABB_AABBjs();
    this.m_shape.computeAABB(aabb1, xf1, proxy.childIndex);
    this.m_shape.computeAABB(aabb2, xf2, proxy.childIndex);

    proxy.aabb.combine(aabb1, aabb2);

    var displacement = commonVec2_Vec2js.sub(xf2.p, xf1.p);
