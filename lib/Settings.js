'use strict';

Object.defineProperty(exports, "__esModule", {
  value: true
});
var exportObject = {};
var Settings_angularSleepToleranceSqr;
var Settings_angularSleepTolerance;
var Settings_linearSleepToleranceSqr;
var Settings_linearSleepTolerance;
var Settings_timeToSleep;
var Settings_toiBaugarte;
var Settings_baumgarte;
var Settings_maxRotationSquared;
var Settings_maxRotation;
var Settings_maxTranslationSquared;
var Settings_maxTranslation;
var Settings_maxAngularCorrection;
var Settings_maxLinearCorrection;
var Settings_velocityThreshold;
var Settings_maxDistnceIterations;
var Settings_maxTOIIterations;
var Settings_maxTOIContacts;
var Settings_maxSubSteps;
var Settings_polygonRadius;
var Settings_angularSlop;
var Settings_linearSlopSquared;
var Settings_linearSlop;
var Settings_aabbMultiplier;
var Settings_aabbExtension;
var Settings_maxPolygonVertices;
var Settings_maxManifoldPoints;
/*
 * Copyright (c) 2016-2018 Ali Shakiba http://shakiba.me/planck.js
 * Copyright (c) 2006-2011 Erin Catto  http://www.box2d.org
 *
 * This software is provided 'as-is', without any express or implied
 * warranty.  In no event will the authors be held liable for any damages
 * arising from the use of this software.
 * Permission is granted to anyone to use this software for any purpose,
 * including commercial applications, and to alter it and redistribute it
 * freely, subject to the following restrictions:
 * 1. The origin of this software must not be misrepresented; you must not
 * claim that you wrote the original software. If you use this software
 * in a product, an acknowledgment in the product documentation would be
 * appreciated but is not required.
 * 2. Altered source versions must be plainly marked as such, and must not be
 * misrepresented as being the original software.
 * 3. This notice may not be removed or altered from any source distribution.
 */

var _DEBUG = typeof DEBUG === 'undefined' ? false : DEBUG;
var _ASSERT = typeof ASSERT === 'undefined' ? false : ASSERT;

// TODO merge with World options?

var Settings = exportObject;

/**
 * Tuning constants based on meters-kilograms-seconds (MKS) units.
 */

// Collision
/**
 * The maximum number of contact points between two convex shapes. Do not change
 * this value.
 */
exports.maxManifoldPoints = Settings_maxManifoldPoints = 2;

/**
 * The maximum number of vertices on a convex polygon. You cannot increase this
 * too much because BlockAllocator has a maximum object size.
 */
exports.maxPolygonVertices = Settings_maxPolygonVertices = 12;

/**
 * This is used to fatten AABBs in the dynamic tree. This allows proxies to move
 * by a small amount without triggering a tree adjustment. This is in meters.
 */
exports.aabbExtension = Settings_aabbExtension = 0.1;

/**
 * This is used to fatten AABBs in the dynamic tree. This is used to predict the
 * future position based on the current displacement. This is a dimensionless
 * multiplier.
 */
exports.aabbMultiplier = Settings_aabbMultiplier = 2.0;

/**
 * A small length used as a collision and constraint tolerance. Usually it is
 * chosen to be numerically significant, but visually insignificant.
 */
exports.linearSlop = Settings_linearSlop = 0.005;
exports.linearSlopSquared = Settings_linearSlopSquared = Settings_linearSlop * Settings_linearSlop;

/**
 * A small angle used as a collision and constraint tolerance. Usually it is
 * chosen to be numerically significant, but visually insignificant.
 */
exports.angularSlop = Settings_angularSlop = 2.0 / 180.0 * Math.PI;

/**
 * The radius of the polygon/edge shape skin. This should not be modified.
 * Making this smaller means polygons will have an insufficient buffer for
 * continuous collision. Making it larger may create artifacts for vertex
 * collision.
 */
exports.polygonRadius = Settings_polygonRadius = 2.0 * Settings_linearSlop;

/**
 * Maximum number of sub-steps per contact in continuous physics simulation.
 */
exports.maxSubSteps = Settings_maxSubSteps = 8;

// Dynamics

/**
 * Maximum number of contacts to be handled to solve a TOI impact.
 */
Settings_maxTOIContacts = 32;

/**
 * Maximum iterations to solve a TOI.
 */
exports.maxTOIIterations = Settings_maxTOIIterations = 20;

/**
 * Maximum iterations to find Distance.
 */
exports.maxDistnceIterations = Settings_maxDistnceIterations = 20;

/**
 * A velocity threshold for elastic collisions. Any collision with a relative
 * linear velocity below this threshold will be treated as inelastic.
 */
exports.velocityThreshold = Settings_velocityThreshold = 1.0;

/**
 * The maximum linear position correction used when solving constraints. This
 * helps to prevent overshoot.
 */
exports.maxLinearCorrection = Settings_maxLinearCorrection = 0.2;

/**
 * The maximum angular position correction used when solving constraints. This
 * helps to prevent overshoot.
 */
exports.maxAngularCorrection = Settings_maxAngularCorrection = 8.0 / 180.0 * Math.PI;

/**
 * The maximum linear velocity of a body. This limit is very large and is used
 * to prevent numerical problems. You shouldn't need to adjust this.
 */
exports.maxTranslation = Settings_maxTranslation = 2.0;
exports.maxTranslationSquared = Settings_maxTranslationSquared = Settings_maxTranslation * Settings_maxTranslation;

/**
 * The maximum angular velocity of a body. This limit is very large and is used
 * to prevent numerical problems. You shouldn't need to adjust this.
 */
exports.maxRotation = Settings_maxRotation = 0.5 * Math.PI;
exports.maxRotationSquared = Settings_maxRotationSquared = Settings_maxRotation * Settings_maxRotation;

/**
 * This scale factor controls how fast overlap is resolved. Ideally this would
 * be 1 so that overlap is removed in one time step. However using values close
 * to 1 often lead to overshoot.
 */
exports.baumgarte = Settings_baumgarte = 0.2;
exports.toiBaugarte = Settings_toiBaugarte = 0.75;

// Sleep

/**
 * The time that a body must be still before it will go to sleep.
 */
exports.timeToSleep = Settings_timeToSleep = 0.5;

/**
 * A body cannot sleep if its linear velocity is above this tolerance.
 */
Settings_linearSleepTolerance = 0.01;

exports.linearSleepToleranceSqr = Settings_linearSleepToleranceSqr = Math.pow(Settings_linearSleepTolerance, 2);

/**
 * A body cannot sleep if its angular velocity is above this tolerance.
 */
Settings_angularSleepTolerance = 2.0 / 180.0 * Math.PI;

exports.angularSleepToleranceSqr = Settings_angularSleepToleranceSqr = Math.pow(Settings_angularSleepTolerance, 2);

exports.maxManifoldPoints = Settings_maxManifoldPoints;
exports.maxPolygonVertices = Settings_maxPolygonVertices;
exports.aabbExtension = Settings_aabbExtension;
exports.aabbMultiplier = Settings_aabbMultiplier;
exports.linearSlop = Settings_linearSlop;
exports.linearSlopSquared = Settings_linearSlopSquared;
exports.angularSlop = Settings_angularSlop;
exports.polygonRadius = Settings_polygonRadius;
exports.maxSubSteps = Settings_maxSubSteps;
exports.maxTOIIterations = Settings_maxTOIIterations;
exports.maxDistnceIterations = Settings_maxDistnceIterations;
exports.velocityThreshold = Settings_velocityThreshold;
exports.maxLinearCorrection = Settings_maxLinearCorrection;
exports.maxAngularCorrection = Settings_maxAngularCorrection;
exports.maxTranslation = Settings_maxTranslation;
exports.maxTranslationSquared = Settings_maxTranslationSquared;
exports.maxRotation = Settings_maxRotation;
exports.maxRotationSquared = Settings_maxRotationSquared;
exports.baumgarte = Settings_baumgarte;
exports.toiBaugarte = Settings_toiBaugarte;
exports.timeToSleep = Settings_timeToSleep;
exports.linearSleepToleranceSqr = Settings_linearSleepToleranceSqr;
exports.angularSleepToleranceSqr = Settings_angularSleepToleranceSqr;