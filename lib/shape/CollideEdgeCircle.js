...
function EdgeCircleContact(manifold, xfA, fixtureA, indexA, xfB, fixtureB,
    indexB) {
  _ASSERT && utilcommon_assertjs.assert(fixtureA.getType() == EdgeShape_EdgeShapejs.TYPE);
  _ASSERT && utilcommon_assertjs.assert(fixtureB.getType() == CircleShape_CircleShapejs.TYPE);
...
function ChainCircleContact(manifold, xfA, fixtureA, indexA, xfB, fixtureB,
    indexB) {
  _ASSERT && utilcommon_assertjs.assert(fixtureA.getType() == ChainShape_ChainShapejs.TYPE);
  _ASSERT && utilcommon_assertjs.assert(fixtureB.getType() == CircleShape_CircleShapejs.TYPE);
...
function CollideEdgeCircle(manifold, edgeA, xfA, circleB, xfB) {
  ...
  _ASSERT && utilcommon_assertjs.assert(den > 0.0);
  ...
  // manifold.points[0].id.key = 0;
  manifold.points[0].id.cf.indexA = 0;
  manifold.points[0].id.cf.typeA = Manifold_Manifoldjs.e_face;
  manifold.points[0].id.cf.indexB = 0;
  manifold.points[0].id.cf.typeB = Manifold_Manifoldjs.e_vertex;
}
