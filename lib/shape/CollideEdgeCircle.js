...
_Contact.Contact.addType(_EdgeShape.TYPE, _CircleShape.TYPE, EdgeCircleContact);
_Contact.Contact.addType(_ChainShape.TYPE, _CircleShape.TYPE, ChainCircleContact);

function EdgeCircleContact(manifold, xfA, fixtureA, indexA, xfB, fixtureB, indexB) {
  _ASSERT && (0, _common.assert)(fixtureA.getType() == _EdgeShape.TYPE);
  _ASSERT && (0, _common.assert)(fixtureB.getType() == _CircleShape.TYPE);
...
function ChainCircleContact(manifold, xfA, fixtureA, indexA, xfB, fixtureB, indexB) {
  _ASSERT && (0, _common.assert)(fixtureA.getType() == _ChainShape.TYPE);
  _ASSERT && (0, _common.assert)(fixtureB.getType() == _CircleShape.TYPE);
...
function CollideEdgeCircle(manifold, edgeA, xfA, circleB, xfB) {
  ...
  // manifold.points[0].id.key = 0;
  manifold.points[0].id.cf.indexA = 0;
  manifold.points[0].id.cf.typeA = _Manifold.Manifold.e_face;
  manifold.points[0].id.cf.indexB = 0;
  manifold.points[0].id.cf.typeB = _Manifold.Manifold.e_vertex;
}