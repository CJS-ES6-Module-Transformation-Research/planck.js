...
function EdgePolygonContact(manifold, xfA, fA, indexA, xfB, fB, indexB) {
  _ASSERT && utilcommon_assertjs.assert(fA.getType() == EdgeShape_EdgeShapejs.TYPE);
  _ASSERT && utilcommon_assertjs.assert(fB.getType() == PolygonShape_PolygonShapejs.TYPE);

  CollideEdgePolygon(manifold, fA.getShape(), xfA, fB.getShape(), xfB);
}

function ChainPolygonContact(manifold, xfA, fA, indexA, xfB, fB, indexB) {
  _ASSERT && utilcommon_assertjs.assert(fA.getType() == ChainShape_ChainShapejs.TYPE);
  _ASSERT && utilcommon_assertjs.assert(fB.getType() == PolygonShape_PolygonShapejs.TYPE);
...
function CollideEdgePolygon(manifold, edgeA, xfA, polygonB, xfB) {
  ...
  var radius = 2.0 * Settings_Settingsjs.polygonRadius;
  ...
  { // ComputePolygonSeparation
    ...
    for (var i = 0; i < polygonBA.count; ++i) {
      ...
      if (commonVec2_Vec2js.dot(n, perp) >= 0.0) {
        if (commonVec2_Vec2js.dot(commonVec2_Vec2js.sub(n, upperLimit), normal) < -Settings_Settingsjs.angularSlop) {
          continue;
        }
      } else {
        if (commonVec2_Vec2js.dot(commonVec2_Vec2js.sub(n, lowerLimit), normal) < -Settings_Settingsjs.angularSlop) {
          continue;
        }
      }
  ...
  var ie = [ new Manifold_Manifoldjs.clipVertex(), new Manifold_Manifoldjs.clipVertex() ];
  ...
  if (primaryAxis.type == e_edgeA) {
    ...
    ie[0].id.cf.typeA = Manifold_Manifoldjs.e_face;
    ie[0].id.cf.typeB = Manifold_Manifoldjs.e_vertex;

    ie[1].v = polygonBA.vertices[i2];
    ie[1].id.cf.indexA = 0;
    ie[1].id.cf.indexB = i2;
    ie[1].id.cf.typeA = Manifold_Manifoldjs.e_face;
    ie[1].id.cf.typeB = Manifold_Manifoldjs.e_vertex;
    ...
    ie[0].id.cf.typeA = Manifold_Manifoldjs.e_vertex;
    ie[0].id.cf.typeB = Manifold_Manifoldjs.e_face;

    ie[1].v = v2;
    ie[1].id.cf.indexA = 0;
    ie[1].id.cf.indexB = primaryAxis.index;
    ie[1].id.cf.typeA = Manifold_Manifoldjs.e_vertex;
    ie[1].id.cf.typeB = Manifold_Manifoldjs.e_face;
  ...
  // Clip to box side 1
  np = Manifold_Manifoldjs.clipSegmentToLine(clipPoints1, ie, rf.sideNormal1, rf.sideOffset1, rf.i1);

  if (np < Settings_Settingsjs.maxManifoldPoints) {
    return;
  }

  // Clip to negative box side 1
  np = Manifold_Manifoldjs.clipSegmentToLine(clipPoints2, clipPoints1, rf.sideNormal2, rf.sideOffset2, rf.i2);
  ...
  var pointCount = 0;
  for (var i = 0; i < Settings_Settingsjs.maxManifoldPoints; ++i) {
