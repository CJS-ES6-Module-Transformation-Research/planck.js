...
ChainShape.prototype._createLoop = function (vertices) {
  _ASSERT && _common.assert.assert(this.m_vertices.length == 0 && this.m_count == 0);
  _ASSERT && _common.assert.assert(vertices.length >= 3);
  for (var i = 1; i < vertices.length; ++i) {
    var v1 = vertices[i - 1];
    var v2 = vertices[i];
    // If the code crashes here, it means your vertices are too close together.
    _ASSERT && _common.assert.assert((0, _Vec.distanceSquared)(v1, v2) > _Settings.linearSlopSquared);
  }
...
ChainShape.prototype._createChain = function (vertices) {
  _ASSERT && _common.assert.assert(this.m_vertices.length == 0 && this.m_count == 0);
  _ASSERT && _common.assert.assert(vertices.length >= 2);
  for (var i = 1; i < vertices.length; ++i) {
    // If the code crashes here, it means your vertices are too close together.
    var v1 = vertices[i - 1];
    var v2 = vertices[i];
...
ChainShape.prototype.getChildEdge = function (edge, childIndex) {
  _ASSERT && _common.assert.assert(0 <= childIndex && childIndex < this.m_count - 1);
...
ChainShape.prototype.rayCast = function (output, input, xf, childIndex) {
  _ASSERT && _common.assert.assert(0 <= childIndex && childIndex < this.m_count);
...
ChainShape.prototype.computeAABB = function (aabb, xf, childIndex) {
  ...
  var v1 = (0, _Transform.mulVec2)(xf, this.getVertex(childIndex));
  var v2 = (0, _Transform.mulVec2)(xf, this.getVertex(childIndex + 1));
 ...
 */
exports.ChainShape = exported_ChainShape;
