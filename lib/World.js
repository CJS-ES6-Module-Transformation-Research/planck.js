...
function World(def) {
  ...
  this.m_gravity = (0, _Vec.clone)(def.gravity);
...
World.prototype.rayCast = function (point1, point2, reportFixtureCallback) {
  _ASSERT && _common.assert.assert(typeof reportFixtureCallback === 'function');
  var broadPhase = this.m_broadPhase;
  ...
  this.m_broadPhase.rayCast({
    ...
    if (hit) {
      ...
      var point = (0, _Vec.add)((0, _Vec.mul)(1.0 - fraction, input.p1), (0, _Vec.mul)(fraction, input.p2));
...
World.prototype.shiftOrigin = function (newOrigin) {
  _ASSERT && _common.assert.assert(this.m_locked == false);
  if (this.m_locked) {
    return;
  }
...
World.prototype.createBody = function (def, angle) {
  _ASSERT && _common.assert.assert(this.isLocked() == false);
  if (this.isLocked()) {
    return null;
  }
...
World.prototype.destroyBody = function (b) {
  _ASSERT && _common.assert.assert(this.m_bodyCount > 0);
  _ASSERT && _common.assert.assert(this.isLocked() == false);
  if (this.isLocked()) {
    return;
  }
...
World.prototype.createJoint = function (joint) {
  _ASSERT && _common.assert.assert(!!joint.m_bodyA);
  _ASSERT && _common.assert.assert(!!joint.m_bodyB);
  _ASSERT && _common.assert.assert(this.isLocked() == false);
  if (this.isLocked()) {
    return null;
  }
...
World.prototype.destroyJoint = function (joint) {
  ...
  _ASSERT && _common.assert.assert(this.m_jointCount > 0);
  --this.m_jointCount;
...
World.prototype.createContact = function (proxyA, proxyB) {
  ...
  // Call the factory.
  var contact = (0, _Contact.create)(fixtureA, indexA, fixtureB, indexB);
  if (contact == null) {
    return;
  }
...
World.prototype.destroyContact = function (contact) {
  (0, _Contact.destroy)(contact, this);
