...
var _BroadPhase = require("./collision/BroadPhase");
...
function World(def) {
  ...
  if (def && _Vec.Vec2.isValid(def)) {
    def = { gravity: def };
  }

  def = (0, _options.optionsjs)(def, WorldDef);

  this.m_solver = new _Solver.Solver(this);

  this.m_broadPhase = new _BroadPhase.BroadPhase();
  ...
  this.m_gravity = _Vec.Vec2.clone(def.gravity);
...
World.prototype.rayCast = function (point1, point2, reportFixtureCallback) {
  ...
  this.m_broadPhase.rayCast({
    ...
    if (hit) {
      ...
      var point = _Vec.Vec2.add(_Vec.Vec2.mul(1.0 - fraction, input.p1), _Vec.Vec2.mul(fraction, input.p2));
...
World.prototype.createBody = function (def, angle) {
  ...
  var body = new _Body.Body(this, def);
...
var s_step = new _Solver.Solver.TimeStep(); // reuse
...
World.prototype.createContact = function (proxyA, proxyB) {
  ...
  // Call the factory.
  var contact = _Contact.Contact.create(fixtureA, indexA, fixtureB, indexB);
  if (contact == null) {
    return;
  }
...
World.prototype.destroyContact = function (contact) {
  _Contact.Contact.destroy(contact, this);
