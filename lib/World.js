import { optionsjs as World_options } from "./util/options";
import { assert as World_common } from "./util/common";
...
import { BroadPhase as World_BroadPhase } from "./collision/BroadPhase";
import { Solver as World_Solver, TimeStep as Solverjs_TimeStep } from "./Solver";
import { Body as World_Body } from "./Body";

import {
  Contact as World_Contact,
  create as Contactjs_create,
  destroy as Contactjs_destroy,
} from "./Contact";
...
function World(def) {
  ...
  if (def && Vec2js_isValid(def)) {
    def = {gravity : def};
  }

  def = World_options(def, WorldDef);

  this.m_solver = new World_Solver(this);

  this.m_broadPhase = new World_BroadPhase();
  ...
  this.m_gravity = Vec2js_clone(def.gravity);
...
World.prototype.queryAABB = function(aabb, queryCallback) {
  _ASSERT && World_common.assert(typeof queryCallback === 'function');
...
World.prototype.rayCast = function(point1, point2, reportFixtureCallback) {
  _ASSERT && World_common.assert(typeof reportFixtureCallback === 'function');
  var broadPhase = this.m_broadPhase;
  ...
  this.m_broadPhase.rayCast({
    ...
    if (hit) {
      ...
      var point = Vec2js_add(Vec2js_mul((1.0 - fraction), input.p1), Vec2js_mul(fraction, input.p2));
...
World.prototype.shiftOrigin = function(newOrigin) {
  _ASSERT && World_common.assert(this.m_locked == false);
  if (this.m_locked) {
    return;
  }
...
World.prototype.createBody = function(def, angle) {
  _ASSERT && World_common.assert(this.isLocked() == false);
  if (this.isLocked()) {
    return null;
  }
  ...
  var body = new World_Body(this, def);
...
World.prototype.destroyBody = function(b) {
  _ASSERT && World_common.assert(this.m_bodyCount > 0);
  _ASSERT && World_common.assert(this.isLocked() == false);
  if (this.isLocked()) {
    return;
  }
...
World.prototype.createJoint = function(joint) {
  _ASSERT && World_common.assert(!!joint.m_bodyA);
  _ASSERT && World_common.assert(!!joint.m_bodyB);
  _ASSERT && World_common.assert(this.isLocked() == false);
  if (this.isLocked()) {
    return null;
  }
...
World.prototype.destroyJoint = function(joint) {
  ...
  _ASSERT && World_common.assert(this.m_jointCount > 0);
  --this.m_jointCount;
...
var s_step = new Solverjs_TimeStep(); // reuse
...
World.prototype.createContact = function(proxyA, proxyB) {
  ...
  // Call the factory.
  var contact = Contactjs_create(fixtureA, indexA, fixtureB, indexB);
  if (contact == null) {
    return;
  }
...
World.prototype.destroyContact = function(contact) {
  Contactjs_destroy(contact, this);
