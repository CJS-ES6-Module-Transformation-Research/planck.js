...
import { BroadPhase as collisionBroadPhase_BroadPhasejs } from "./collision/BroadPhase";
import { Solver as Solver_Solverjs } from "./Solver";
 ...
 */
var WorldDef = {
  gravity : commonVec2_Vec2js.zero(),
  allowSleep : true,
  warmStarting : true,
...
function World(def) {
  ...
  if (def && commonVec2_Vec2js.isValid(def)) {
    def = {gravity : def};
  }

  def = utiloptions_optionsjsjs(def, WorldDef);

  this.m_solver = new Solver_Solverjs(this);

  this.m_broadPhase = new collisionBroadPhase_BroadPhasejs();
  ...
  this.m_gravity = commonVec2_Vec2js.clone(def.gravity);
...
World.prototype.rayCast = function(point1, point2, reportFixtureCallback) {
  _ASSERT && utilcommon_assertjs.assert(typeof reportFixtureCallback === 'function');
  var broadPhase = this.m_broadPhase;
  ...
  this.m_broadPhase.rayCast({
    ...
    if (hit) {
      ...
      var point = commonVec2_Vec2js.add(commonVec2_Vec2js.mul((1.0 - fraction), input.p1), commonVec2_Vec2js.mul(fraction, input.p2));
...
World.prototype.shiftOrigin = function(newOrigin) {
  _ASSERT && utilcommon_assertjs.assert(this.m_locked == false);
  if (this.m_locked) {
    return;
  }
...
World.prototype.createBody = function(def, angle) {
  _ASSERT && utilcommon_assertjs.assert(this.isLocked() == false);
  if (this.isLocked()) {
    return null;
  }
  ...
  var body = new Body_Bodyjs(this, def);
...
World.prototype.createDynamicBody = function(def, angle) {
  if (!def) {
    def = {};
  } else if (commonVec2_Vec2js.isValid(def)) {
...
World.prototype.destroyBody = function(b) {
  _ASSERT && utilcommon_assertjs.assert(this.m_bodyCount > 0);
  _ASSERT && utilcommon_assertjs.assert(this.isLocked() == false);
  if (this.isLocked()) {
    return;
  }
...
World.prototype.createJoint = function(joint) {
  _ASSERT && utilcommon_assertjs.assert(!!joint.m_bodyA);
  _ASSERT && utilcommon_assertjs.assert(!!joint.m_bodyB);
  _ASSERT && utilcommon_assertjs.assert(this.isLocked() == false);
  if (this.isLocked()) {
    return null;
  }
...
World.prototype.destroyJoint = function(joint) {
  ...
  _ASSERT && utilcommon_assertjs.assert(this.m_jointCount > 0);
  --this.m_jointCount;
...
var s_step = new Solver_Solverjs.TimeStep(); // reuse
...
World.prototype.createContact = function(proxyA, proxyB) {
  ...
  // Call the factory.
  var contact = Contact_Contactjs.create(fixtureA, indexA, fixtureB, indexB);
  if (contact == null) {
    return;
  }
...
World.prototype.destroyContact = function(contact) {
  Contact_Contactjs.destroy(contact, this);
