...
function Manifold() {
  this.type;
  this.localNormal = _Vec2.default.zero();
  this.localPoint = _Vec2.default.zero();
...
Manifold.prototype.getWorldManifold = function (wm, xfA, radiusA, xfB, radiusB) {
  ...
  switch (this.type) {
    case Manifold.e_circles:
      normal = _Vec2.default.neo(1.0, 0.0);
      var pointA = _Transform2.default.mulVec2(xfA, this.localPoint);
      var pointB = _Transform2.default.mulVec2(xfB, this.points[0].localPoint);
      var dist = _Vec2.default.sub(pointB, pointA);
      if (_Vec2.default.lengthSquared(dist) > _Math2.default.EPSILON * _Math2.default.EPSILON) {
        normal.set(dist);
        normal.normalize();
      }
      points[0] = _Vec2.default.mid(pointA, pointB);
    ...
    case Manifold.e_faceA:
      normal = _Rot2.default.mulVec2(xfA.q, this.localNormal);
      var planePoint = _Transform2.default.mulVec2(xfA, this.localPoint);

      for (var i = 0; i < this.pointCount; ++i) {
        var clipPoint = _Transform2.default.mulVec2(xfB, this.points[i].localPoint);
        var cA = _Vec2.default.clone(clipPoint).addMul(radiusA - _Vec2.default.dot(_Vec2.default.sub(clipPoint, planePoint), normal), normal);
        var cB = _Vec2.default.clone(clipPoint).subMul(radiusB, normal);
        points[i] = _Vec2.default.mid(cA, cB);
        separations[i] = _Vec2.default.dot(_Vec2.default.sub(cB, cA), normal);
      }
    ...
    case Manifold.e_faceB:
      normal = _Rot2.default.mulVec2(xfB.q, this.localNormal);
      var planePoint = _Transform2.default.mulVec2(xfB, this.localPoint);

      for (var i = 0; i < this.pointCount; ++i) {
        var clipPoint = _Transform2.default.mulVec2(xfA, this.points[i].localPoint);
        var cB = _Vec2.default.combine(1, clipPoint, radiusB - _Vec2.default.dot(_Vec2.default.sub(clipPoint, planePoint), normal), normal);
        var cA = _Vec2.default.combine(1, clipPoint, -radiusA, normal);
        points[i] = _Vec2.default.mid(cA, cB);
        separations[i] = _Vec2.default.dot(_Vec2.default.sub(cA, cB), normal);
      }
      points.length = this.pointCount;
      separations.length = this.pointCount;
...
function clipSegmentToLine(vOut, vIn, normal, offset, vertexIndexA) {
  ...
  var distance0 = _Vec2.default.dot(normal, vIn[0].v) - offset;
  var distance1 = _Vec2.default.dot(normal, vIn[1].v) - offset;
